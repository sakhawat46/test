const items = [
  { num: "01", label: "$0 Due Until A Doctor Approves You" },
  { num: "02", label: "U.S. Board-Certified Physicians" },
  { num: "03", label: "OBGYNs on Your Care Team" },
  { num: "04", label: "Doctor Approval in 24 Hours or Less" },
];

export default function BadgesNumbered() {
  return (
          ...
          <div className="pl-4 flex flex-col gap-2.5">
            {items.map(({ num, label }) => (
              <div key={num} className="flex items-start gap-2">
                <span className="font-serif text-[16px] font-bold leading-none mt-0.5 flex-shrink-0" style={{ color: "#F0327A" }}>{num}</span>
                <p className="text-[11px] font-semibold text-[#38404B] leading-[1.3] uppercase tracking-[0.04em]">{label}</p>
              </div>
            ))}
          </div>
