...
const items = [
  { icon: DollarSign, label: "$0 Due Until Doctor Approves", iconBg: "#4EB7C0" },
  { icon: Stethoscope, label: "Board-Certified Physicians", iconBg: "#F0327A" },
  { icon: UserCheck, label: "OBGYNs on Your Care Team", iconBg: "#4EB7C0" },
  { icon: Clock, label: "Approval in 24 Hours or Less", iconBg: "#F0327A" },
];

export default function BadgesIconSquare() {
  return (
    <div className="min-h-screen bg-[#f5eef4] flex items-center justify-center p-3">
      <div className="w-[390px] bg-white rounded-3xl overflow-hidden" style={{ boxShadow: "0 24px 80px rgba(78,183,192,0.16), 0 4px 20px rgba(240,50,122,0.09)" }}>
        ...
        >
          {/* Left column: 2×2 icon square cards */}
          <div className="pl-2 grid grid-cols-2 gap-2">
            {items.map(({ icon: Icon, label, iconBg }) => (
              <div
                key={label}
                className="bg-white rounded-2xl p-2.5 flex flex-col items-center text-center gap-2"
              ...
              >
                ...
                <p className="text-[9.5px] font-semibold text-[#38404B] leading-[1.25]">{label}</p>
              </div>
            ))}
          </div>
