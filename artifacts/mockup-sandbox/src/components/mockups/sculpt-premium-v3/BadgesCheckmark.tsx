...
export default function BadgesCheckmark() {
  return (
    <div className="min-h-screen bg-[#f5eef4] flex items-center justify-center p-3">
      <div className="w-[390px] bg-white rounded-3xl overflow-hidden" style={{ boxShadow: "0 24px 80px rgba(78,183,192,0.16), 0 4px 20px rgba(240,50,122,0.09)" }}>
        ...
        >
          {/* Left column: checkmark list */}
          <div className="pl-5 flex flex-col gap-3">
            {items.map(({ label, color }) => (
              <div key={label} className="flex items-start gap-2.5">
                <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ backgroundColor: color }}>
                  <Check className="w-3 h-3 text-white" strokeWidth={3} />
                </div>
                <p className="text-[12px] font-medium text-[#38404B] leading-[1.3]">{label}</p>
              </div>
            ))}
          </div>
