  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", height: "100vh", background: "linear-gradient(135deg, #faf7ff 0%, #fff0f6 100%)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "8px 20px 0", position: "relative", overflow: "hidden" }}>
        ...
        <img src={VIAL} alt="Vial" style={{ position: "absolute", right: -51, bottom: 160, width: "58%", objectFit: "contain", WebkitMaskImage: "linear-gradient(to bottom, black 77%, transparent 100%)", maskImage: "linear-gradient(to bottom, black 77%, transparent 100%)" }} />

        {/* TRUST — V1: stacked plain text rows */}
        <div style={{ position: "absolute", bottom: 24, left: 20, right: 20, display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {badges.map((b, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: i === 0 ? 700 : 500, color: i === 0 ? ink : "#64748b" }}>{b}</span>
            </div>
          ))}
        </div>
      </div>
