...
const badges = ["Physician-led", "Board-certified MDs", "Licensed US pharmacies"];
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", height: "100vh", background: "linear-gradient(135deg, #faf7ff 0%, #fff0f6 100%)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "8px 20px 0", position: "relative", overflow: "hidden" }}>
        ...
        <img src={VIAL} alt="Vial" style={{ position: "absolute", right: -51, bottom: 172, width: "58%", objectFit: "contain", WebkitMaskImage: "linear-gradient(to bottom, black 77%, transparent 100%)", maskImage: "linear-gradient(to bottom, black 77%, transparent 100%)" }} />

        {/* TRUST — V2: stacked pill badges */}
        <div style={{ position: "absolute", bottom: 24, left: 20, right: 20, display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {badges.map((b, i) => (
            <div key={i} style={{
              ...
              background: "#fff", border: `1px solid ${teal}28`, borderRadius: 50,
              padding: "6px 14px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)"
            }}>
              <svg style={{ width: 10, height: 10, flexShrink: 0 }} fill="none" viewBox="0 0 24 24" stroke={teal} strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
              <span style={{ fontSize: 12.5, fontWeight: 600, color: ink, whiteSpace: "nowrap" }}>{b}</span>
            </div>
          ))}
        </div>
      </div>
